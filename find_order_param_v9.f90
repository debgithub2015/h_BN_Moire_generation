...
! Find the order parameter
        ...
        DEALLOCATE(rx_init, ry_init, rz_init, rx_fin, ry_fin, rz_fin, &
                ...
                hexa_l1,b_fin,hexa_rcom_l1,hexa_rcom_l2,&
                hexa_norm_l1) 
...
! Lower layer hexagon points
      ...
      ! d_rcom=SQRT(temp_x**2+temp_y**2+temp_z**2)
       ...
       length_scale=SQRT(((1.d0+SQRT(3.d0))*a0)**2)

       IF(d_rcom.lt.2.54d0) THEN

       WRITE(rcom_output,*) ii,d_rcom

       WRITE(6,*) ii,rcom_hexa_l2(:),no_atom_found(ii,:),&
                   d_rcom,d_rcom/length_scale
